...
# Alpine Linux SOCKS5 (dante-server) 交互式安装脚本 (V2)
# 智汇 (Gemini)
#
# !! 安全警告 !!
...
# 2. 获取用户输入
echo "--- SOCKS5 代理 (Dante) 交互式安装程序 ---"

read -p "请输入 SOCKS5 代理端口 (默认 75426): " PROXY_PORT
[ -z "$PROXY_PORT" ] && PROXY_PORT=75426

read -p "请输入 SOCKS5 认证用户名 (回车 = 不设置密码/公开代理): " SOCKS_USER
...
if [ -n "$SOCKS_USER" ]; then
    # 需要认证
    read -sp "请输入 SOCKS5 认证密码: " SOCKS_PASS
    echo
    ...
    echo "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
    echo
    read -p "我理解风险并确认要创建公开代理 (请输入 'yes' 继续): " CONFIRM_OPEN
...
if [ "$AUTH_METHOD" = "username" ]; then
    ...
    # -D (不分配密码), -H (不创建 home 目录), -s /bin/false (禁止登录)
...
if [ -z "$EXT_IF" ]; then
    ...
    EXT_IF=$(ip -4 route ls | grep default | grep -Po '(?<=dev )(\S+)' | head -n1)
    if [ -z "$EXT_IF" ]; then
...
# --- 客户端规则 (谁可以连接到代理) ---
# 允许任何 IP 连接 (如果 socksmethod = username, 则会在此之后要求认证)
client pass {
    from: 0.0.0.0/0
    to: 0.0.0.0/0
    log: connect error  # 记录连接和错误
}
...
echo "2. 如果您的 Alpine 机器配置了防火墙 (例如 apf 或 iptables)，"
echo "   请确保放行您设置的 TCP 端口: $PROXY_PORT"
echo "3. 如需卸载，请运行: "
